...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "swarm_dev" do |swarm_dev|
    swarm_dev.vm.box = $swarm_dev_hv_box
    swarm_dev.vm.box_version = $swarm_dev_hv_box_ver

  	swarm_dev.vm.provider "hyperv" do |hv|
  		hv.vmname = $swarm_dev_vmname
  		# With nested virtualization, at least 2 CPUs are needed.
  		hv.cpus = $swarm_dev_vcpus
  		# With nested virtualization, at least 4GB of memory is needed.
  		hv.memory = $swarm_dev_vmem
      # Faster cloning and uses less disk space
      hv.differencing_disk = true
      # Mac Address
      hv.mac = $swarm_dev_mac
  	end

    swarm_dev.vm.provision "shell", inline: <<-SHELL
